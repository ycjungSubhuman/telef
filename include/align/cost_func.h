...
namespace {
    ...
    Eigen::Matrix<T, 4, 4> buildTransform(const T *rigidCoeff) {
        T yaw = rigidCoeff[0];
    ...
    template <class T>
    Eigen::Matrix<T, Eigen::Dynamic, 1> lmkInd2Pos(const Eigen::Matrix<T, Eigen::Dynamic, 1> &fullMeshPos,
...
namespace telef::align{
    ...
    public:
        explicit PCALandmarkDistanceFunctor(std::vector<int> meshLandmarkInds, CloudConstPtrT scanLandmark3d,
        ...
        bool operator()(const T* pcaCoeff, const T* rigidCoeff, T* residuals) const {
            ...
            Eigen::Matrix<T, Eigen::Dynamic, 1> meshLmk3d = lmkInd2Pos(meshPos, meshLandmarkInds);

            int validPointCount = 0;

            for(unsigned long i=0; i<meshLandmarkInds.size(); i++) {
    ...
    private:
        std::vector<int> meshLandmarkInds;
