...
private:
  ...
  float reg;
  telef::io::MeshNormalDepthRendererWrapper render;

  boost::shared_ptr<PCANonRigidAlignmentSuite>
  _processData(boost::shared_ptr<telef::feature::FittingSuite> in) override;

public:
  explicit MultipleModelFeederPipe(const std::vector<MModelTptr> &model, telef::io::MeshNormalDepthRendererWrapper render, float reg=0.0001);
};
} // namespace telef::face
