...
enum PLY_FORMAT { unknown, ascii, binary_le, binary_be };
...
public:
  ...
  inline SCALAR value(size_t nr) const { return data[nr]; }
...
public:
  ...
  virtual void print_header(std::ostream &ostr) const {
    ostr << "property list " << ply_type<LIST_SCALAR>() << " "
...
public:
  ...
  template <typename TYPE> void setScalars(const char *names, const TYPE *data);

  template <typename TYPE>
  void setList(const char *name, const std::vector<std::vector<TYPE>> &data);

  // gets a referenc to a property with the provided name / type (creates one if
  // not yet existing!)
  ...
  template <class PROPERTY_TYPE> PROPERTY_TYPE *getProperty(const char *name);
...
void PLY_ELEMENT::setScalars(const char *names, const TYPE *data) {
  ...
  for (int ii = 0; ii < nrProperties; ii++) {
    PLY_PROPERTY_SCALAR<TYPE> &property =
        getPropertyCreate<PLY_PROPERTY_SCALAR<TYPE>>(
...
PROPERTY_TYPE &PLY_ELEMENT::getPropertyCreate(const char *name) {
  ...
  properties.emplace_back(
      std::shared_ptr<PROPERTY_TYPE>(new PROPERTY_TYPE(name, nrElements)));
...
bool PLY_ELEMENT::getScalarProperty(const char *name, std::vector<SCALAR> &p) {
  PLY_PROPERTY_SCALAR<SCALAR> *property =
      getProperty<PLY_PROPERTY_SCALAR<SCALAR>>(name);
  if (!property)
    return false;
  p = property->data;
  return true;
...
make_property(const std::string &line, const size_t nrElements) {
  ...
  if (tok.size() == 5 && tok[1].compare("list") == 0) {
    ...
    std::cerr << "Invalid ply file [property definition <" << line << ">]"
              << std::endl;
    return p;
  }
  return p;
...
inline bool PlyFile::load(const std::string &file) {
  ...
  while (std::getline(infile, line)) {
    ...
    if (tok[0].compare("element") == 0) {
      if (tok.size() != 3) {
        std::cerr << "Invalid ply file [element definition <" << line << ">]"
                  << std::endl;
        return false;
      }
    ...
    if (tok[0].compare("property") == 0) {
      if (elements_.size() == 0) {
        std::cerr << "missing element definition before <" << line << ">"
                  << std::endl;
        return false;
      }
