...
namespace telef::io::align {
    ...
    void PCAVisualizerFrontEnd::process(InputPtrT input) {
        //PCL_INFO ("Visualizing Frame...");
        auto modelMesh = input->model;

        if (!visualizer) {
            //PCL_INFO ("Making Visualizer...");
            visualizer = std::make_unique<vis::PCLVisualizer>();
            visualizer->setBackgroundColor(0, 0, 0);
        }
        visualizer->spinOnce();
//        if (!visualizer->updateTextureMesh(*modelMesh, "model_mesh")) {
            //PCL_INFO ("Adding Mesh...");
        if(visualizer->addTextureMesh(*modelMesh, "model_mesh", 0)){
            visualizer->setPosition(0, 0);
            //visualizer->addCoordinateSystem (1.0);
            //visualizer->setSize(modelMesh->cloud.width, modelMesh->cloud.height);
            visualizer->initCameraParameters();
        } else {
            visualizer->removePointCloud("model_mesh", 0);
            visualizer->addTextureMesh(*modelMesh, "model_mesh", 0);
        }
        //PCL_INFO ("Frame Done!"
...
//        }
    ...
    void PCARigidVisualizerFrontEnd::process(InputPtrT input) {
        ...
        ColorMesh meanMesh = input->pca_model->genMesh(Eigen::VectorXf::Zero(50));
