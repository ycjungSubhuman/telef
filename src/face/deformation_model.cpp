...
Eigen::MatrixXf getPCABase(Eigen::MatrixXf data, int maxRank) {
  ...
  // Each row is a data. This is to match data matrix dimensions with formulas
  // in Wikipedia.
  ...
  if (d.rows() <=
      d.cols()) { // singular values are shorter than position dimension
  ...
  for (int i = 0;
       i < std::min(maxRank, static_cast<int>(bdc.singularValues().rows()));
       i++) {
    result.col(i).swap(pairs[i].second);
  }

  // Fill in empty basis if given PCA rank is higher than the rank of the data
  // matrix
  if (maxRank > d.cols()) {
    std::cout << "WARNING : given rank is higher than number of singular values"
              << std::endl;
...
int PCADeformationModel::getRank() { return rank; }

Eigen::VectorXf PCADeformationModel::getCenter() { return mean; }
...
BlendShapeDeformationModel::BlendShapeDeformationModel(
...
int BlendShapeDeformationModel::getRank() { return rank; }
